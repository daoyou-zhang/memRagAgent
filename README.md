# memRagAgent

> 基于 RAG 的智能记忆代理系统，支持多层记忆、知识图谱、用户画像、Prompt 自进化和多租户管理。

## 🎯 设计原则

本项目的设计融合了三种哲学思想：

### 有生于无
> "道生一，一生二，二生三，三生万物"

系统从**原则**（道）出发，生成**领域能力**（有）：
- 代码定义不变的核心原则 → 数据库存储可进化的领域 Prompt
- 新领域无需改代码，只需配置数据库

### 律令
> "你应当只按照那个你同时希望成为普遍法则的准则行动"

`PROMPT_PRINCIPLES` 定义所有 Prompt 必须遵守的普遍准则：
- 身份原则：始终以"道友"自称，不暴露底层模型
- 合规原则：不生成违法、歧视内容
- 向善原则：积极引导，尊重用户

### 实用
> "教育即生长，生长即目的"

**一切设计以用户需求为导向，系统在使用中成长**：
- 根据用户对话自动生成记忆和画像
- 根据用户反馈自进化 Prompt
- 知识库按需扩展，不预设固定领域
- 新增"测试智能体"、"编程助手"等领域只需数据库配置

## 🏗 系统架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                        Frontend (React + Vite)                       │
│                      深色主题 | 侧边栏导航 | 13 个功能页面            │
└─────────────────────────────┬───────────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────────┐
│                      daoyou_agent (:8000)                            │
│             认知控制器 | 工具编排 | 上下文聚合 | 流式回复             │
└────────┬─────────────────────┬─────────────────────┬────────────────┘
         │                     │                     │
┌────────▼──────┐     ┌────────▼──────┐     ┌───────▼───────┐
│ Memory :5000  │     │ Knowledge     │     │   Neo4j       │
│  记忆管理      │     │    :5001      │     │  知识图谱     │
│  用户画像      │     │  知识库 RAG   │     │  实体关系     │
└───────┬───────┘     └───────┬───────┘     └───────────────┘
        │                     │
┌───────▼─────────────────────▼───────────────────────────────────────┐
│                   PostgreSQL | ChromaDB | Redis                      │
│                  关系数据 | 向量检索 | 热点缓存                       │
└─────────────────────────────────────────────────────────────────────┘
```

## ✨ 核心功能

### 1. 多层记忆系统
| 记忆类型 | 说明 | 生命周期 |
|----------|------|----------|
| `episodic` | 情节记忆，对话摘要 | 短期 |
| `semantic` | 语义记忆，用户偏好/事实 | 长期 |
| `working` | 工作记忆，当前会话消息 | 会话级 |
| `profile` | 用户画像，聚合结果 | 持久 |

### 2. 知识库 + 知识图谱
- **知识集合**: 按领域（法律/心理/医疗/话术）组织
- **文档索引**: 支持 txt/pdf/docx/json/jsonl
- **向量检索**: ChromaDB 高性能向量搜索
- **图谱增强**: Neo4j 实体关系 + 图谱 RAG

### 3. 多租户支持
- **租户隔离**: Tenant → UserGroup → User 层级
- **API 密钥**: 每租户独立 API Key 管理
- **数据隔离**: project_id/tenant_id 字段贯穿所有表
- **详细文档**: 参见 [TENANT_SECURITY.md](./TENANT_SECURITY.md)

### 4. 性能优化
- **Redis 缓存**: Embedding/Profile/RAG 结果缓存
- **连接池**: PostgreSQL pool_size=10, max_overflow=20
- **批量操作**: ChromaDB 批量向量写入

### 5. Prompt 自进化系统

```
┌──────────────────┐
│ PROMPT_PRINCIPLES│  ← 代码定义（不变）
│   核心原则        │     身份/合规/向善/专业/交互
└────────┬─────────┘
         │ 生成
┌────────▼─────────┐
│  prompt_configs  │  ← 数据库存储（可进化）
│   领域 Prompt     │     八字/法律/测试/编程...
└────────┬─────────┘
         │ 进化
┌────────▼─────────┐
│ prompt_evolution │  ← 自动建议
│   进化历史        │     LLM 返回 → 审批 → 应用
└──────────────────┘
```

**优先级**: 请求参数 > 数据库 project > 数据库 category > 代码原则

## 📁 项目结构

```
memRagAgent/
├── backend/
│   ├── daoyou_agent/        # 认知代理服务 (:8000)
│   │   ├── api/             # API 路由
│   │   ├── config/
│   │   │   └── prompts.py   # Prompt 原则（道）
│   │   ├── services/        # 核心服务
│   │   │   ├── cognitive_controller.py   # 认知控制器
│   │   │   ├── prompt_service.py         # Prompt CRUD
│   │   │   ├── context_aggregator.py     # 上下文聚合
│   │   │   └── tool_orchestrator.py      # 工具编排
│   │   └── tools/           # MCP 工具
│   │
│   ├── memory/              # 记忆服务 (:5000)
│   │   ├── routes/          # API 路由
│   │   │   ├── memories.py  # 记忆 CRUD
│   │   │   ├── profiles.py  # 用户画像
│   │   │   ├── rag.py       # RAG 检索
│   │   │   └── tenants.py   # 租户管理
│   │   ├── services/
│   │   │   └── prompt_evolution.py  # Prompt 自进化
│   │   └── models/          # ORM 模型
│   │
│   ├── knowledge/           # 知识库服务 (:5001)
│   │   ├── routes/
│   │   │   ├── knowledge.py # 集合/文档/分块
│   │   │   └── graph.py     # 图谱操作
│   │   └── services/
│   │       └── graph_service.py  # 图谱增强 RAG
│   │
│   ├── shared/              # 共享模块
│   │   ├── vector_store.py  # ChromaDB 封装
│   │   └── cache.py         # Redis 缓存
│   │
│   └── .env                 # 统一配置
│
└── frontend/                # React 前端
    └── src/
        ├── pages/           # 13 个功能页面
        ├── components/      # 可复用组件
        └── api/             # API 客户端
```

## 🚀 快速开始

### 1. 环境要求
- Python 3.11+
- Node.js 18+
- PostgreSQL 14+
- Redis 7+
- Neo4j 5+ (可选)

### 2. 后端启动

```bash
cd backend

# 安装依赖
pip install -r requirements.txt
pip install -r requirements.txt --no-index --find-links=./local-wheels

# 配置环境变量
cp .env.example .env
# 编辑 .env 填写数据库、API Key 等配置

# 初始化数据库（按顺序执行）
psql -d memrag -f memory/db/init_all.sql      # Memory 服务所有表
psql -d memrag -f knowledge/db/init_all.sql   # Knowledge 服务所有表

# 或者如果需要清空重建
# DROP SCHEMA public CASCADE; CREATE SCHEMA public;
# 然后重新执行上面的 SQL

# 启动服务（分别在三个终端）
cd memory && python app.py      # :5000
cd knowledge && python app.py   # :5001
cd daoyou_agent && python app.py # :8000
```

### 3. 前端启动

```bash
cd frontend

npm install
npm run dev
# 访问 http://localhost:5173
```

## ⚙️ 配置说明

主要环境变量 (`backend/.env`):

```bash
# ============ 数据库 ============
POSTGRES_HOST=localhost
POSTGRES_PORT=5432
POSTGRES_DB=memrag
POSTGRES_USER=postgres
POSTGRES_PASSWORD=your_password

# ============ Redis ============
REDIS_URL=redis://:password@localhost:6379
REDIS_ENABLED=true
REDIS_CACHE_TTL=300

# ============ Neo4j ============
NEO4J_URI=bolt://localhost:7687
NEO4J_USER=neo4j
NEO4J_PASSWORD=your_password

# ============ ChromaDB ============
USE_CHROMADB=true
CHROMA_PERSIST_DIR=./chroma_data

# ============ LLM ============
MODEL_NAME=deepseek-v3
API_MODEL_KEYS=sk-xxx
API_MODEL_BASE=https://api.example.com/v1

# ============ Embedding ============
EMBEDDINGS_NAME=embedding-3
API_EMBEDDINGS_KEY=xxx
API_EMBEDDINGS_BASE=https://api.example.com/v4

# ============ 服务地址 ============
MEMORY_SERVICE_URL=http://127.0.0.1:5000
KNOWLEDGE_SERVICE_URL=http://127.0.0.1:5001
```

## 📡 API 概览

### Memory Service (:5000)

| 方法 | 路径 | 说明 | 缓存 |
|------|------|------|------|
| POST | `/api/memory/memories` | 创建记忆 | - |
| GET | `/api/memory/memories` | 查询记忆列表 | - |
| POST | `/api/memory/context/full` | 获取完整上下文 | - |
| POST | `/api/memory/jobs/unified` | 创建统一记忆生成任务 | - |
| GET | `/api/profiles/:user_id` | 获取用户画像 | ✅ Redis 10min |
| POST | `/api/rag/query` | RAG 检索 | ✅ Redis 5min |
| POST | `/api/tenants` | 创建租户 | - |
| GET | `/api/tenants` | 获取租户列表 | - |
| POST | `/api/tenants/:id/groups` | 创建用户组 | - |
| POST | `/api/tenants/:id/users` | 创建用户 | - |
| POST | `/api/auth/login` | 用户登录 | - |

### Knowledge Service (:5001)

| 方法 | 路径 | 说明 | 缓存 |
|------|------|------|------|
| GET | `/api/knowledge/collections` | 列出知识集合 | - |
| POST | `/api/knowledge/collections` | 创建知识集合 | - |
| GET | `/api/knowledge/documents` | 列出文档 | - |
| POST | `/api/knowledge/documents` | 创建文档 | - |
| POST | `/api/knowledge/documents/:id/index` | 索引文档 | - |
| POST | `/api/knowledge/rag/query` | 知识库 RAG | ✅ Redis 5min |
| POST | `/api/knowledge/graph/search` | 图谱实体搜索 | ✅ Redis 10min |
| POST | `/api/knowledge/graph/neighbors` | 获取实体邻居 | ✅ Redis 10min |
| POST | `/api/knowledge/graph/path` | 查找路径 | - |
| POST | `/api/knowledge/graph/extract` | 从文本抽取实体 | - |
| POST | `/api/knowledge/graph/query` | 执行 Cypher | - |

### Agent Service (:8000)

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/cognitive/process` | 认知对话（支持流式） |
| GET | `/health` | 健康检查 |

**认知对话请求示例**:

```json
{
  "input": "帮我看看这个代码有什么问题",
  "user_id": "user123",
  "session_id": "session456",
  "project_id": "myproject",
  "stream": true,
  
  // 可选：用户自定义模型
  "model_config_override": {
    "api_base": "https://api.openai.com/v1",
    "model_name": "gpt-4",
    "api_keys": "sk-xxx"
  }
}
```

**内置工具**:
- `bazi_paipan` - 八字排盘
- `read_file` - 读取代码文件
- `list_directory` - 列出目录结构

## 🎯 功能页面（15 个）

| 页面 | 路径 | 功能 |
|------|------|------|
| 首页 | `/` | 系统概览 |
| 认知对话 | `/cognitive` | AI 对话界面（流式） |
| 创建记忆 | `/memories/create` | 手动创建记忆 |
| 查询记忆 | `/memories/query` | 搜索记忆 |
| 记忆清理 | `/memories/cleanup` | 批量清理 |
| 生成任务 | `/jobs` | 任务管理 |
| 用户画像 | `/profiles` | 画像查看 |
| Memory RAG | `/rag` | 记忆 RAG 测试 |
| Full Context | `/full-context` | 上下文测试 |
| 知识集合 | `/knowledge/collections` | 集合管理 |
| 知识文档 | `/knowledge/documents` | 文档管理 |
| Knowledge RAG | `/knowledge/rag` | 知识库 RAG |
| 知识图谱 | `/graph` | 图谱可视化 |
| 多租户管理 | `/tenants` | 租户/用户组/用户 |
| 系统状态 | `/system` | 服务监控和功能测试 |

## 📊 数据模型

### 记忆相关
- `memories` - 记忆存储
- `conversation_sessions` - 会话
- `conversation_messages` - 消息
- `profiles` - 用户画像
- `memory_generation_jobs` - 生成任务

### 知识库相关
- `knowledge_collections` - 知识集合
- `knowledge_documents` - 知识文档
- `knowledge_chunks` - 知识分块

### 多租户相关
- `tenants` - 租户
- `user_groups` - 用户组
- `users` - 用户
- `api_keys` - API 密钥

## 🔧 开发路线

### 已完成 ✅
- [x] 基础记忆 CRUD
- [x] 语义记忆提取
- [x] 用户画像聚合
- [x] ChromaDB 向量检索
- [x] 知识库 RAG + Redis 缓存
- [x] Neo4j 图谱集成
- [x] 图谱增强 RAG + Redis 缓存
- [x] 多租户支持
- [x] Redis 缓存优化（画像/RAG/图谱）
- [x] 前端深色主题（15 个页面）
- [x] 用户自定义模型配置
- [x] MCP 工具系统（文件读取）
- [x] 统一日志（loguru）
- [x] 配置模板 (.env.example)

### 待完成 🚧
- [x] API Key 认证与租户隔离
- [x] 认证与隔离测试 (test_auth.py)
- [ ] 单元测试覆盖
- [ ] API 文档 (OpenAPI/Swagger)
- [ ] Docker 部署脚本
- [ ] 代码沙箱执行
- [ ] 文件上传解析

## 🧪 测试

### 运行测试

```bash
cd backend/tests

# 运行所有认证测试
pytest test_auth.py -v

# 只运行管理员模式测试
pytest test_auth.py::TestAdminMode -v

# 只运行租户隔离测试
pytest test_auth.py::TestTenantIsolation -v

# 只运行租户管理 API 测试
pytest test_auth.py::TestTenantManagement -v
```

### 测试配置

测试使用 `test_config.py` 集中管理配置：

```python
from test_config import (
    ADMIN,           # 管理员配置
    TENANT_A,        # 测试租户 A (DAOYOUTEST)
    TENANT_B,        # 测试租户 B
    TEST_USER_1,     # 测试用户 1
    get_admin_headers,
    get_user_headers,
)
```

### 测试模式

| 模式 | 环境变量 | 说明 |
|------|----------|------|
| 开发模式 | `AUTH_ENABLED=false` | 无需认证，所有请求有管理员权限 |
| 生产模式 | `AUTH_ENABLED=true` | 需要有效 API Key |

## 📄 License

MIT

